...
$GIT_CLONE $UCD_SERVICE_GATEWAY_REPO_URL.git \
  --branch $UCD_SERVICE_GATEWAY_REPO_TAG \
  --depth 1 \
  $REPOSITORY_DIR/$UCD_SERVICE_GATEWAY_REPO_NAME

# UCD Client
$GIT_CLONE $UCD_CLIENT_REPO_URL.git \
  --branch $UCD_CLIENT_REPO_TAG \
  --depth 1 \
  $REPOSITORY_DIR/$UCD_CLIENT_REPO_NAME

# UCD VIVO harvester
$GIT_CLONE $VIVO_HARVESTER_REPO_URL.git \
  --branch $VIVO_HARVESTER_REPO_TAG \
  --depth 1 \
  $REPOSITORY_DIR/$VIVO_HARVESTER_REPO_NAME