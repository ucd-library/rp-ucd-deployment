...
gcloud builds submit \
  ...
  --substitutions=_UCD_LIB_INITIATOR=$USER,REPO_NAME=$(basename $(git remote get-url origin) .git),TAG_NAME=$(git describe --tags --abbrev=0),BRANCH_NAME=$(git rev-parse --abbrev-ref HEAD),SHORT_SHA=$(git log -1 --pretty=%h) \
  .