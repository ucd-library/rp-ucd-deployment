...
if [[ ! -z $LOCAL_BUILD ]]; then
  ...
  UCD_VIVO_REPO_TAG='local-dev'
  UCD_SERVICE_GATEWAY_REPO_TAG='local-dev'
  VIVO_HARVESTER_REPO_TAG='local-dev'
  UCD_CLIENT_REPO_TAG='local-dev'
  FUSEKI_REPO_TAG='local-dev'
fi
...
VIVO_HARVESTER_REPO_HASH=$(git -C $REPOSITORY_DIR/$VIVO_HARVESTER_REPO_NAME log -1 --pretty=%h)
UCD_CLIENT_REPO_HASH=$(git -C $REPOSITORY_DIR/$UCD_CLIENT_REPO_NAME log -1 --pretty=%h)
...
##
docker build \
  -t $UCD_LIB_DOCKER_ORG/$FUSEKI_IMAGE_NAME:$FUSEKI_REPO_TAG \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
  $REPOSITORY_DIR/$FUSEKI_REPO_NAME/jena-fuseki
  # --cache-from $UCD_LIB_DOCKER_ORG/$FUSEKI_IMAGE_NAME:$DOCKER_CACHE_TAG \
...
# VIVO Harvester
docker build \
  -t $UCD_LIB_DOCKER_ORG/$VIVO_HARVESTER_IMAGE_NAME:$VIVO_HARVESTER_REPO_TAG \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
  $REPOSITORY_DIR/$VIVO_HARVESTER_REPO_NAME
  # --cache-from $UCD_LIB_DOCKER_ORG/$VIVO_HARVESTER_IMAGE_NAME:$DOCKER_CACHE_TAG \
...
docker build \
  -t $UCD_LIB_DOCKER_ORG/$UCD_SERVICE_GATEWAY_IMAGE_NAME:$UCD_SERVICE_GATEWAY_REPO_TAG \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
...
docker build \
  -t $UCD_LIB_DOCKER_ORG/$UCD_CLIENT_IMAGE_NAME:$UCD_CLIENT_REPO_TAG \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
  $REPOSITORY_DIR/$UCD_CLIENT_IMAGE_NAME
  # --cache-from $UCD_LIB_DOCKER_ORG/$UCD_CLIENT_IMAGE_NAME:$DOCKER_CACHE_TAG \