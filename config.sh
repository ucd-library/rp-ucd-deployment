...
APP_VERSION=v1.6.0-alpha.${BUILD_NUM}
...
HARVEST_IMAGE_NAME=$CONTAINER_REG_ORG/$HARVEST_REPO_NAME
CLIENT_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-client
NODE_UTILS_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-node-utils
DEBOUNCER_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-kafka-debouncer
INDEXER_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-es-indexer
MODEL_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-es-models
API_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-api
GATEWAY_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-gateway
AUTH_IMAGE_NAME=$CONTAINER_REG_ORG/rp-ucd-auth-cas
KAFKA_INIT_IMAGE_NAME=$CONTAINER_REG_ORG/rp-kafka-init
STATUS_IMAGE_NAME=$CONTAINER_REG_ORG/rp-index-status
...
STATUS_IMAGE_NAME_TAG=$STATUS_IMAGE_NAME:$VESSEL_TAG
...
ALL_DOCKER_BUILD_IMAGES=( $CLIENT_IMAGE_NAME $DEBOUNCER_IMAGE_NAME \
 ...
 $AUTH_IMAGE_NAME $HARVEST_IMAGE_NAME $MODEL_IMAGE_NAME $STATUS_IMAGE_NAME $KAFKA_INIT_IMAGE_NAME )
...
ALL_DOCKER_BUILD_IMAGE_TAGS=( $HARVEST_IMAGE_NAME_TAG $CLIENT_IMAGE_NAME_TAG \
 ...
 $AUTH_IMAGE_NAME_TAG $STATUS_IMAGE_NAME_TAG $KAFKA_INIT_IMAGE_NAME_TAG )
